...
We are focussed only on API servers here
```
...
```notes
Build node.js server that returns some JSON
Ask the class for a theme
```
...
```notes
Every node package has a package.json
```
...
```notes
Specify the node packages we want to use
